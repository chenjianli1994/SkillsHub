...
| `requirement-only` | 只做需求分析、需求整理、需求规格书或需求拆解 | 0 -> 1 -> 2 -> 3 -> 10（按用户目标可只执行其中一个阶段） |
...
- 编写业务流程和异常流程。
- 编写功能需求。
- 为功能需求补充上游来源编号、需求类型、建议验证方式、追踪状态和是否进入软件需求拆解。
- 编写数据需求。
- 编写非功能需求。
- 编写验收标准。
- 建立需求追踪关系。
- 同步生成 `requirement-specification.json`，作为需求拆解和后续设计的结构化输入。
- 保留待确认事项。
...
- 每条功能需求是否有编号。
- 每条需求是否有验收标准。
- 每条功能需求是否保留上游来源编号、需求类型、建议验证方式、追踪状态和拆解标记。
- 是否存在无法测试的描述。
- 是否存在与业务规则冲突的内容。
- 是否保留来源和确认依据。
...
按汽车电子软件开发中的 ASPICE SWE.1 软件需求分析风格，把客户需求、系统需求或普通需求规格书拆解为可追踪、可验证、可进入设计的软件需求基线。
...
- 客户需求、系统需求或用户补充的上游需求材料
- 业务优先级
- 版本计划约束

### 要完成的内容

- 建立上游需求基线，保留已有编号；无编号时生成 `UR-*` 代理编号。
- 拆解软件需求清单，主编号使用 `SWR-*`。
- 建立 CM 符合性矩阵，表达上游需求到软件需求的直接承接、拆分、合并、派生或约束继承关系。
- 标识每条软件需求的需求类型、验证方式、优先级、复杂度、依赖和风险。
- 统计覆盖状态：已覆盖、部分覆盖、未覆盖、待确认、不适用。
- 记录一致性/不完整性问题，例如缺参数、缺边界、冲突、不可验证或来源不明。
- 形成后续方案设计和详细设计读取的设计输入集。
...
- 是否每条 `SWR-*` 都能追溯到上游需求编号。
- 是否每条软件需求都有验证方式。
- 是否通过 CM 矩阵覆盖所有功能类上游需求。
- 是否标识覆盖状态、一致性问题、依赖和风险。
- 是否避免把开发任务或实现方案提前写死。
